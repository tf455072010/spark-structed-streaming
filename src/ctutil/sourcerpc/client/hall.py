...
class HallClient:
    ...
    def __connect(self):
        ...
        self.__client = HallService.Client(tMultiplexedProtocol)
    ...
    # @mem.autocache("CTDC.SOURCERPC.AREA.CITIES", 90000000)  # 缓存25小时
    ...
    def listhall(self):
        ...
        Returns： (5000, '景德镇电信', '09', '0904', '')
        '''

        protocs = self.__getclient().allhalls()
        if not protocs:
            raise error.InvalidValueError("无效的大厅列表")
        halls = []
        for protoc in protocs:
            hallid = protoc.hallid
            hallname = protoc.hallname
            province_code = protoc.province
            city_code = protoc.city
            district_code = protoc.district
            hall = (hallid, hallname, province_code, city_code, district_code)
            halls.append(hall)
        return halls
